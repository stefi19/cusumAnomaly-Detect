...
begin
  ...
  begin
    ...
    assert m_axis_result_tdata = expected
      report "Adder TB: test1 failed (0+0)" severity error;
    report "Adder TB: test1 passed (0+0)" severity note;
    s_axis_a_tvalid <= '0';
    s_axis_b_tvalid <= '0';
    ...
    expected := x"00000005";
    assert m_axis_result_tdata = expected
      report "Adder TB: test2 failed (5+0)" severity error;
    report "Adder TB: test2 passed (5+0)" severity note;
    s_axis_a_tvalid <= '0';
    s_axis_b_tvalid <= '0';
    wait until m_axis_result_tvalid = '0';
    wait for 20 ns;

    -- Test 3: 7 + 7 = 14 (same-number test)
    ...
    expected := x"0000000E";
    assert m_axis_result_tdata = expected
      report "Adder TB: test3 failed (7+7)" severity error;
    report "Adder TB: test3 passed (7+7)" severity note;
    s_axis_a_tvalid <= '0';
    s_axis_b_tvalid <= '0';

    wait for 50 ns;
    report "Adder TB: all tests completed" severity note;
    wait;
  end process stim;
